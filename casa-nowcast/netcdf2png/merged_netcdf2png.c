...
void data2picture(float **databuf, struct mergedimensions *datadims,
  ...
  float tmpgrid[201][201];
  for(j=0;j<201;j++){
    for(i=0;i<201;i++){
      tmpgrid[j][i] = databuf[j][i];
    }
  }
  ...
  for (e = 0; e < 3; e++) {
    for (x = smoothvoxels; x<(201-smoothvoxels); x++) {
      for (y = smoothvoxels; y<(201-smoothvoxels); y++) {
	...
	for (z=1; z<smoothvoxels+1; z++)
	  shepard_tot = shepard_tot + ((((1/((z*latspac)*(z*latspac))) * databuf[x-z][y]) + ((1/((z*latspac)*(z*latspac))) * databuf[x+z][y]) + ((1/((z*lonspac)*(z*lonspac))) * databuf[x][y-z]) + ((1/((z*lonspac)*(z*lonspac))) * databuf[x][y+z])) + ((1/((z*crossdx)*(z*crossdx))) * databuf[x-z][y-z]) + ((1/((z*crossdx)*(z*crossdx))) * databuf[x+z][y-z]) + ((1/((z*crossdx)*(z*crossdx))) * databuf[x-z][y+z]) + ((1/((z*crossdx)*(z*crossdx))) * databuf[x+z][y+z]));
	shepard_tot = shepard_tot/shepard_denom;
	if (shepard_k > 0) {
	  shepard = shepard_tot/shepard_k;
	  if (e == 0) {
	    if (tmpgrid[x][y] > 10) {
	      databuf[x][y] = ((3*tmpgrid[x][y]) + shepard)/4;
	    }
	    else {
	      databuf[x][y] = shepard;
	    }
	    ...
	    if (tmpgrid[x][y] > 10) {
	      databuf[x][y] = ((tmpgrid[x][y]) + shepard)/2;
	    }
	    else {
	      databuf[x][y] = shepard;
	    }
  ...
  for(j=0;j<201;j++){
    for(i=0;i<201;i++){
      if (databuf[(201-j)-1][i] == 0)
	continue;
      pix_val=databuf[(201-j)-1][i];
...
int main(int argc, char *argv[]){
  ...
  arguments.width=1006;
  arguments.height=1006;
  ...
  arguments.plotmin=0;
  arguments.plotmax=75;

  /* Parse command line */
  argp_parse(&argp,argc,argv,0,0,&arguments);
    
  if(strlen(arguments.palletname)==0)
    strncpy(arguments.palletname, "/home/ldm/netcdf2png/colorscales/nexrad_ref.png",MAX_NAME);
